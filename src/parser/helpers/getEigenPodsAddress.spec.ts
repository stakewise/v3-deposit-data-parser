...
import getEigenPodsAddress from './getEigenPodsAddress'
import type { GetEigenPodsAddressInput } from './getEigenPodsAddress'
...
describe('getEigenPodsAddress', () => {
  ...
  it('should return the correct operator address for valid input', async () => {
    ...
    const result = await getEigenPodsAddress(input)
  ...
  it('should throw an error if withdrawalAddress is missing', async () => {
    const input = {
      vaultAddress: '0xe05d8895e8b3ba51ce4f89b337c621889d3f38bf',
      network: 'holesky',
    } as GetEigenPodsAddressInput

    const errorText = new ParserError(ErrorTypes.MISSING_FIELDS, { fields: [ 'withdrawal_address' ] })
    await expect(getEigenPodsAddress(input)).rejects.toThrow(errorText)
  })
